...
import (
	...
	"github.com/AetherClawTech/aetherclaw/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to aetherclaw",
		Args:  cobra.NoArgs,
		Example: `  aetherclaw migrate
  aetherclaw migrate --from openclaw
  aetherclaw migrate --dry-run
  aetherclaw migrate --refresh
  aetherclaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.aetherclaw)")
