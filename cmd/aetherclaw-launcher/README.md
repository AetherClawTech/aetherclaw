...
A standalone launcher for AetherClaw, providing visual JSON editing and OAuth provider authentication management.
...
```bash
# Build
go build -o aetherclaw-launcher ./cmd/aetherclaw-launcher/

# Run with default config path (~/.aetherclaw/config.json)
./aetherclaw-launcher

# Specify a config file
./aetherclaw-launcher ./config.json

# Allow LAN access
./aetherclaw-launcher -public
```
...
```
Usage: aetherclaw-config [options] [config.json]

Arguments:
  config.json    Path to the configuration file (default: ~/.aetherclaw/config.json)
...
{
  "config": { ... },
  "path": "/Users/xiao/.aetherclaw/config.json"
}
```
...
After authentication, Google redirects to `GET /auth/callback`, which saves the credentials and redirects back to the aetherclaw-config UI.
...
Gets the running status of the `aetherclaw gateway` process.
...
Starts the `aetherclaw gateway` process in the background.
...
Stops the running `aetherclaw gateway` process.
...
```bash
go test -v ./cmd/aetherclaw-launcher/
```
