...
func (s *appState) modelMenu() tview.Primitive {
	...
	items = append(items,
		...
		MenuItem{
			...
			Action: func() {
				s.addModel(
					aetherclawconfig.ModelConfig{ModelName: "new-model", Model: "openai/gpt-5.2"},
				)
				s.push(
...
func (s *appState) addModel(model aetherclawconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []aetherclawconfig.ModelConfig) {
...
func isModelValid(model aetherclawconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *aetherclawconfig.ModelConfig) {
	if model == nil {
		return
	}
