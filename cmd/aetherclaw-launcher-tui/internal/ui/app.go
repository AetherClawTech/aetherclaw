...
import (
	...
	configstore "github.com/AetherClawTech/aetherclaw/cmd/aetherclaw-launcher-tui/internal/config"
...
type appState struct {
	...
	pages       *tview.Pages
	stack       []string
	config      *aetherclawconfig.Config
	configPath  string
	gatewayCmd  *exec.Cmd
...
func Run() error {
	...
	if cfg == nil {
		cfg = aetherclawconfig.DefaultConfig()
	}
...
func refreshMainMenu(menu *Menu, s *appState) {
	...
	items := []MenuItem{
		...
		{
			Label:       "Start Talk",
			Description: "Open aetherclaw agent in terminal",
			Action: func() {
				s.requestStartTalk()
			},
			Disabled: !modelReady,
...
func (s *appState) startTalk() {
	...
	s.app.Suspend(func() {
		cmd := exec.Command("aetherclaw", "agent")
		cmd.Stdin = os.Stdin
...
func (s *appState) startGateway() {
	...
	cmd := exec.Command("aetherclaw", "gateway")
