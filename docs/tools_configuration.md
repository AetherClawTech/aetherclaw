...
AetherClaw's tools configuration is located in the `tools` field of `config.json`.
...
By default, AetherClaw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `AETHERCLAW_TOOLS_<SECTION>_<KEY>`:

For example:
- `AETHERCLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `AETHERCLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `AETHERCLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
