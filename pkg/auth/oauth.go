...
// Set AETHERCLAW_ANTIGRAVITY_CLIENT_ID and AETHERCLAW_ANTIGRAVITY_CLIENT_SECRET env vars.
func GoogleAntigravityOAuthConfig() OAuthProviderConfig {
	clientID := os.Getenv("AETHERCLAW_ANTIGRAVITY_CLIENT_ID")
	clientSecret := os.Getenv("AETHERCLAW_ANTIGRAVITY_CLIENT_SECRET")
	return OAuthProviderConfig{
...
func buildAuthorizeURL(cfg OAuthProviderConfig, pkce PKCECodes, state, redirectURI string) string {
	...
	if isGoogle {
		...
		if strings.Contains(strings.ToLower(cfg.Issuer), "auth.openai.com") {
			params.Set("originator", "aetherclaw")
		}
		if cfg.Originator != "" {
