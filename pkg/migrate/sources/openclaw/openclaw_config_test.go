...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.aetherclaw/workspace" {
		t.Errorf("expected workspace '~/.aetherclaw/workspace', got '%s'", workspace)
	}
...
func TestConvertToAetherClaw(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToAetherClaw("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.aetherclaw/workspace" {
		t.Errorf("expected workspace '~/.aetherclaw/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToAetherClawWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToAetherClaw("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestToStandardConfig(t *testing.T) {
	picoCfg := &AetherClawConfig{
		Agents: AgentsConfig{
			Defaults: AgentDefaults{
				Provider:  "anthropic",
				ModelName: "claude-sonnet-4-20250514",
				Workspace: "~/.aetherclaw/workspace",
			},
			List: []AgentConfig{
				{
					ID:      "main",
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.aetherclaw/workspace" {
		t.Errorf("expected workspace '~/.aetherclaw/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
