...
func (c *OpenClawConfig) ConvertToAetherClaw(sourceHome string) (*AetherClawConfig, []string, error) {
	cfg := &AetherClawConfig{}
	var warnings []string
	...
	if c.HasSkills() {
		warnings = append(
			warnings,
			fmt.Sprintf(
				"Skills (%d entries) not automatically migrated - reinstall via aetherclaw CLI",
	...
	if c.HasCron() {
		warnings = append(
			warnings,
			"Cron job scheduling not supported in AetherClaw - consider using external schedulers",
		)
	}
	if c.HasHooks() {
		warnings = append(warnings, "Webhook hooks not supported in AetherClaw - use event system instead")
	}
	if c.HasSession() {
		warnings = append(warnings, "Session scope config differs - AetherClaw uses per-agent sessions by default")
	}
...
func (c *OpenClawConfig) convertChannels(warnings *[]string) ChannelsConfig {
	...
	if c.Channels.Signal != nil {
		*warnings = append(*warnings, "Channel 'signal': No AetherClaw adapter available")
	}
	if c.Channels.Matrix != nil {
		*warnings = append(*warnings, "Channel 'matrix': No AetherClaw adapter available")
	}
	if c.Channels.IRC != nil {
		*warnings = append(*warnings, "Channel 'irc': No AetherClaw adapter available")
	}
	if c.Channels.Mattermost != nil {
		*warnings = append(*warnings, "Channel 'mattermost': No AetherClaw adapter available")
	}
	...
	if c.Channels.BlueBubbles != nil {
		*warnings = append(
			*warnings,
			"Channel 'bluebubbles': No AetherClaw adapter available - consider iMessage instead",
		)
	}
...
func (c *AetherClawConfig) ToStandardConfig() *config.Config {
