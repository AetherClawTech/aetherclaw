...
func (p *CodexCliProvider) SetMCPConfigs(configs []MCPProviderConfig) {
	p.mcpConfigs = configs
	p.mcpConfigWritten = false
}
...
func (p *CodexCliProvider) Chat(
	...
	// Write .codex/config.toml if MCP configs are set and workspace is available
	if len(p.mcpConfigs) > 0 && p.workspace != "" && !p.mcpConfigWritten {
		if err := p.writeCodexMCPConfig(); err != nil {
			return nil, fmt.Errorf("failed to write codex MCP config: %w", err)
		}
...
// writeCodexMCPConfig writes a .codex/config.toml in the workspace with MCP server entries.
func (p *CodexCliProvider) writeCodexMCPConfig() error {
	codexDir := filepath.Join(p.workspace, ".codex")
	if err := os.MkdirAll(codexDir, 0o755); err != nil {
		return fmt.Errorf("create .codex dir: %w", err)
	}

	var sb strings.Builder
	sb.WriteString("# Generated by AetherClaw - MCP server configurations\n\n")
